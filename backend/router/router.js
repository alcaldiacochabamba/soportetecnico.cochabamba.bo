...
const auth = require("../controller/auth"); // controlador de autenticación

const multer = require('multer');
const path = require('path');

// Configuración de Multer para subir imágenes PNG
...
// RUTAS DE USUARIOS
//------------------------------------------------------
router.post("/user", authMiddleware([1,2,3]), upload.single('image'), user.store);
router.get("/users", authMiddleware([1,2,3]), user.getAll);
router.get("/user/:usuarios_id", authMiddleware([1,2,3]), user.show);
router.put("/user/:usuarios_id", authMiddleware([1,2,3]), upload.single('image'), user.update);
router.delete("/user/:usuarios_id", authMiddleware([1]), user.destroy);
